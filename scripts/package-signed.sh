#!/usr/bin/env bash
# Build "Band Pass Filter Controller.app" with the BPF ExtensionKit extension EMBEDDED and
# Developer-ID signed, then build the DMG and (when notary creds are present) notarize + staple it.
#
...
#
# Stamp the app version into Resources/Info.plist BEFORE running this (the release workflow's
# "Stamp app version" step does that with PlistBuddy); build-app.sh reads it as-is.
#
#   VERSION=v0.5.0 ./scripts/package-signed.sh   # VERSION names the DMG; pass it verbatim
#
# Env — signing (from CI secrets):
#   MACOS_CERT_P12_BASE64   base64 of the BPF Developer ID Application .p12 (cert + key)
...
APPEX_NAME="BPFExtension.appex"
ENTITLEMENTS="Xcode/Extension/BPF.entitlements"
PROJECT="Xcode/BandPassFilterControllerPlugin.xcodeproj"
SCHEME="BPFExtension"
