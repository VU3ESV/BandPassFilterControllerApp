# Release signing secrets (Band Pass Filter Controller)

The release workflow embeds the BPF ExtensionKit extension into
`Band Pass Filter Controller.app`, signs it with Developer ID, and notarizes the DMG — so an
...
| Secret | What it is |
|--------|------------|
| `MACOS_CERT_P12_BASE64` | BPF's **Developer ID Application** cert+key exported as a `.p12`, base64-encoded |
...
Keychain Access → **login** keychain → **My Certificates** → find BPF's
`Developer ID Application` (its private key is named e.g. `ARS BPF`) → right-click →
**Export…** → `.p12`, set a password (that's `MACOS_CERT_PASSWORD`). Then:

```sh
base64 -i BPF.p12 | pbcopy   # paste as MACOS_CERT_P12_BASE64
```

(Tip: the BPF cert's SHA-1 is `6CABDD1FCBF56B550E980B9E852E9DA995E93FD4` if you need to pick it
